...
function Broadcaster() {
    ...
    this.newClient = function(client, isrepeater) {
        ...
        if (isrepeater) {
          client.on('data', function(msg) { broadcastClients(msg, client); });
        } else {
          client.on('data', function(msg) { broadcaster.broadcast(msg, client); });    
        }
    ...
    this.broadcast = function(msg, source) {
        ...
        try {
            for (n in clients)
                (clients[n] != source) && clients[n].write(msg);
        } 
        catch (ex) {
    ...
    function broadcastClients(msg, source) {
        ...
        try {
            for (n in clients)
                (clients[n] != source) && !clients[n].repeater && clients[n].write(msg);
        } 
        catch (ex) {
            console.log(ex.toString());
        }
    ...
    this.connect = function(port, host, isrepeater) {
        ...
        var client = sm.createClient(socket);
...
exports.createClient = function(port, host) {
    return sm.createClient(port, host);
}
